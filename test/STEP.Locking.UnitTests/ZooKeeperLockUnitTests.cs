...
namespace STEP.Locking.UnitTests
{
    using ZooKeeper;

    public class ZooKeeperLockUnitTests
    {
        const string CONNECTION_STRING = "127.0.0.1:2181";
        ...
        public void LockTest()
        {
            ...
            using (ILockFactory lockFactory = new ZooKeeperLockFactory(CONNECTION_STRING))
            {
                lockFactory.DefaultTimeout = TimeSpan.FromSeconds(5);
                Assert.Equal(LockScopes.DistributedCluster, lockFactory.Scope);
